...
class Rug:
    ...
    def batch_processing(filename, background='area', correct_dispersion='eye', offset=True):
       ...
       # print('\nLen of old batch:', count)
        ...
        processed_files = glob.glob('*processed.dat')
        #allfiles = []
        zipfiles = []
        ...
        with ZipFile('aarhus_TA_jan24.zip', 'r') as zipObj:
            for entry in zipObj.infolist():
                ...
                zipfiles.append(entry.filename)
                
        #newfiles = allfiles.copy()
        ...
        for stuff in zipfiles:
            #print('\nstuff', stuff)
            correctname = stuff[11:-4]
            #print('\ncorrect name', correctname)
            corrected.append(correctname)
                #print(correctname)

        matrixfiles = []
        probe = ['SHG', 'FUN']
        requirements = ['nm', 'matrix']
        probecheck = []

        for names in corrected:
            if any(x in names for x in probe):
                probecheck.append(names)

        for names2 in probecheck:
            if all(y in names2 for y in requirements):
                matrixfiles.append(names2)
        ...
        #stuff that needs to be removed as its already processed
        remove = []
        for repeat in removed_dupes:
            for processes in processed_files:
                if repeat in processes:
                    remove.append(repeat)
                    
        #print('\nRemove', remove, len(remove))
        even_cleaner_batch = cleanedbatch.copy()
        matches = []
        
        for all_the_lists in cleanedbatch:
            for matches in remove:
                for x in range(all_the_lists.count(matches)):
                    all_the_lists.remove(matches)
     
                    
 
        #print('\nall the lists:',  cleanedbatch)
      
                        
                
        """           
        for all_the_lists in cleanedbatch:
            for elements in all_the_lists:
                if elements in 
                if any(elements in elements for matches in remove):
                    print('elements:', elements)
                    all_the_lists.remove(elements)
        """
        for all_the_lists in cleanedbatch:
            #print(all_the_lists)
            if len(all_the_lists) == 1:
                #print(all_the_lists)
                even_cleaner_batch.remove(all_the_lists)
               
        #print('\nRemoved stuff:', even_cleaner_batch)
        
        for groupedfiles in even_cleaner_batch:
