...
class RugTools:
 ...
 #############################
    ...
    def file_sort(filename):
        ...
        print('\033[0;34mEvery single file from spreadsheet:\033[0m\n', dictvalues)
...
class Rug:
    ...
    def batch_processing(filename, background='area', correct_dispersion='eye', offset=True):
        ...
        cleanedbatch = batch
        print('\n\033[0;36mCleaned batch (excludes chirpless files - for nowww\033[0m\n', cleanedbatch, '\n', 'Length:', len(cleanedbatch))
       ...
       # Written to show that some files aren't being retrived - only the last group?
        count = 0
        for groupedfiles_testing in cleanedbatch:
            solventfile_testing = groupedfiles_testing[0]
            groupedfiles_testing.pop(0)
            samplefiles_testing = groupedfiles_testing
            ...
            for files in allfiles:
                if solventfile_testing in files:
                    exactsolventfile_testing = files
                    
                    exactsolventfile.append(exactsolventfile_testing)
                for sample in samplefiles_testing:
                    if sample in files:
                        exactsamplefiles_testing = files
                        excelsamplefiles.append(sample)
                        exactsamplefiles.append(exactsamplefiles_testing)
                        
            print('\n\033[0;31m1. Exact solvent file:\033[0m', exactsolventfile, '\n\033[0;31mExact sample files:\033[0m', exactsamplefiles, '\n\033[0;34m2. Excel solvent file:\033[0m', solventfile_testing, '\n\033[0;34mExcel sample files:\033[0m\n', excelsamplefiles, '\n')
            
            count += 1
        print('How many grouped extractions it has done:', count)
        ...
        for groupedfiles in cleanedbatch:
            ...
            for files in allfiles:
                if solventfile in files:
                    #print('True', files, solventfile)
                    solvent = files
                    #exactsolventfile.append(solvent)
                    solvent = solvent[:-4]
                ...
                for sample in samplefiles:
                    ...
                    #else:
                        #print(sample, 'Not found in directory'
